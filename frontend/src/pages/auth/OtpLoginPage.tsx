...
import { CalendarDays, Loader2, Moon, RotateCcw, Sun } from 'lucide-react';
...
export const OTPLoginPage = () => {
  ...
  // Guard: if someone navigates here directly without the required state, send them back
  useEffect(() => {
    if (!state?.userId) {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      navigate(
        loggedInUser ? getDashboardPath(loggedInUser.role) : '/login',
        { replace: true },
      );
    } catch (err: any) {
      const message = err?.response?.data?.error ?? err?.response?.data?.message ?? 'Kode OTP tidak valid';
      toast.error(message);
  ...
  const handleResend = async () => {
    ...
    setIsResending(true);
    try {
      await authService.resendLoginOTP(state.userId);
      toast.success('Kode OTP baru telah dikirim ke email Anda');
      setCooldown(RESEND_COOLDOWN);
      setDigits(Array(OTP_LENGTH).fill(''));
      inputRefs.current[0]?.focus();
    } catch (err: any) {
      const message = err?.response?.data?.error ?? 'Gagal mengirim ulang OTP';
      toast.error(message);
    } finally {
