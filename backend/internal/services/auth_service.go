...
func (s *AuthService) ValidateCredentials(input LoginInput) (*models.User, error) {
	...
	if dbErr == nil {
		...
		case models.AuthTypeLDAP:
			...
			// Track changes for selective update
			needsUpdate := false

			// Sync Name
			if user.Name != ldapInfo.Name {
				user.Name = ldapInfo.Name
				needsUpdate = true
			}

			// Sync Division (ptr-to-string)
			if ldapInfo.Division != "" {
				if user.Division == nil {
					user.Division = &ldapInfo.Division
					needsUpdate = true
				} else if *user.Division != ldapInfo.Division {
					*user.Division = ldapInfo.Division
					needsUpdate = true
				}
			}

			// Sync Email (only if different AND new email not taken by another user)
			if ldapInfo.Email != "" && ldapInfo.Email != user.Email {
				// Check if new email is available
				existingWithNewEmail, checkErr := s.userRepo.FindByEmail(ldapInfo.Email)
				if checkErr != nil {
					if errors.Is(checkErr, gorm.ErrRecordNotFound) {
						// New email is available, safe to update
						user.Email = ldapInfo.Email
						needsUpdate = true
					} else {
						// Some other DB error, log but don't break login
						log.Printf("Failed to check email availability for %s: %v", ldapInfo.Email, checkErr)
					}
				} else {
					// Email already taken by another user, cannot update
					log.Printf("Cannot sync email for user %d: email %s already in use by user %d",
						user.ID, ldapInfo.Email, existingWithNewEmail.ID)
				}
			}
			...
			if needsUpdate {
				if err := s.userRepo.Update(user); err != nil {
					log.Printf("Failed to update user %d from LDAP: %v", user.ID, err)
				}
			}
	...
	// Try to create with ON CONFLICT handling to avoid race condition
	// If two goroutines try to create the same email simultaneously,
	// the second one will fail with duplicate key and we fall back to FindByEmail.
	err = s.userRepo.Create(newUser)
	if err != nil {
		// Check if it's a duplicate key error (email already exists)
		// In that case, another concurrent request created the user already
		if strings.Contains(err.Error(), "duplicate key") || strings.Contains(err.Error(), "UNIQUE constraint") {
			log.Printf("User with email %s already exists (race condition), fetching existing record", email)
			existingUser, fetchErr := s.userRepo.FindByEmail(email)
			if fetchErr != nil {
	...
	if _, err := s.userRepo.GetOrCreatePreferences(newUser.ID); err != nil {
		...
		log.Printf("Failed to create preferences for new LDAP user %d: %v", newUser.ID, err)
	}
